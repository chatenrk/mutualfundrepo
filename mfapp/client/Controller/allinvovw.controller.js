sap.ui.define([
	...
	var PageController = Controller.extend("simple_hello.Controller.allinvovw", {

		// Setup crumb info, the collection root
		// and an initial product selection / order state
		sCollection: "/FundHierarchy",
		aCrumbs: ["AMC","AssetTypes", "Goals", "funds"],
		mInitialOrderState: {
			products: {},
			count: 0,
			hasCounts: false
      ...
      var sPath;
			...
			// var sPath = jQuery.sap.getModulePath("simple_hello.models", "/allinvmodel.json");
			// var oModel = new JSONModel(sPath);
			// this.getView().setModel(oModel);
			// this.getView().setModel(new JSONModel(this.mInitialOrderState), "Order");
      var oModel = this.getOwnerComponent().getModel("allinvdemomodel");
      var odemodata = oModel.getData();
			if (!this.oTemplate) {
				this.oTemplate = sap.ui.xmlfragment("simple_hello.view.Row");
			}
